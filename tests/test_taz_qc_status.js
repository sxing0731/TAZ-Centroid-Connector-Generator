...
const { getTazStatus, markTazEdited } = statusFactory(state, { cleanId }, ["FLAG", "EDITED", "REVIEWED"]);

assert.equal(getTazStatus("1"), "FLAG", "default TAZ status should be FLAG");
state.importedCc = new Map([["1", [{ nodeId: "20" }, { nodeId: "10" }]]]);
assert.equal(getTazStatus("1"), "FLAG", "same uploaded A/B pairs should remain FLAG");
...
const data = JSON.parse(fs.readFileSync(path.join(root, "docs", "data", "all.json"), "utf8"));
